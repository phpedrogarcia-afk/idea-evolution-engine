...
class M054ExperimentExecutor:
    ...
    def _validate_model_routing(self) -> None:
        ...
        for stage in expected_stages:
            ...
            if model_def.provider != expected_provider:
                raise RuntimeError(
                    f"Condition B stage {stage} provider mismatch: expected {expected_provider}, got {model_def.provider}"
                )
            if model_def.model != expected_model:
                raise RuntimeError(
        ...
        lean_runner = LeanLoopRunner(
            ...
            runs_dir=self._get_temp_dir(),  # Use temporary directory for validation
        )
        ...
        if expected_model == "default-model":
            raise RuntimeError(
                f"Expected model should not be 'default-model', but got: {expected_model}"
            )
